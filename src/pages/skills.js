...
export default (props) => {
  const data = useStaticQuery(graphql`
          query  {
            site {
              siteMetadata {
                  skills {name, level, experience}
              }
            }
          }
        `)
  const skills = data.site.siteMetadata.skills.map((item, index)=> <Skill key={index} name={item.name} level={item.level} experience={item.experience} />)
  return (
